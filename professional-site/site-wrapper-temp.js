...
function installReadabilityGuard(){
  ...
  const observer=new MutationObserver(()=>frames().forEach(cleanFrame));
  const stage=document.querySelector('.workspace-stage');
  if(stage)observer.observe(stage,{subtree:true,childList:true,attributes:true,attributeFilter:['class','src']});
}
var core=document.createElement('script');
core.src='./site-core.js?v=20260827-native-modules-2';
core.async=false;
...
core.onerror=function(){console.error('[Rafal Wilk AI] site core failed to load');};
